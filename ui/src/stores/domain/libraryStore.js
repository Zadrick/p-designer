...
import { OnePatternModel, libraryProjectModel, criteriaModel, LibraryModel } from '../models'

const LibraryStore = types
.model({
    _libraryList: types.optional(types.array(criteriaModel), []),
    _library: types.optional(LibraryModel, {}),
    _libraryProject: types.optional(libraryProjectModel, {}),
...
.actions(self => {
    ...
    const getLibraryProjects = async id => {
        try {
            const res = await fetch(`${apiV}api/libraries?libraryId=${id}&page=1&pageSize=120`).then(res => res.json())
